...
CANDIDATE="rapfi_distillation/round6_regret_restart/training/r6_value_a025_eval.pt"
CANDIDATE_SHA256="1d6888038da07f20456459b69d5284cdba26380a2b1df2754c572963aa79e042"
RUN="rapfi_distillation/round6_regret_restart"
FINAL="$RUN/value_final_seed20280604"
PAIRS=1024
SEED=20280604
...
run_report() {
  local name="$1" checkpoint="$2" target="$FINAL/$1"
  mkdir -p "$target"
  if [[ ! -s "$target/complete.done" ]]; then
    "$PYTHON_BIN" -u -m alphazero_training.rapfi_distill \
      ...
      2>&1 | tee "$RUN/logs/value_final_${name}.log"
