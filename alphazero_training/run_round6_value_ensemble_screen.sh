...
SCREEN="$RUN/value_ensemble_screen_seed20280404"
...
cat > "$RUN/value_ensemble_screen_candidates.tsv" <<'EOF'
r6_value_ensemble_a015	rapfi_distillation/round6_regret_restart/training/r6_value_ensemble_a015_eval.pt
r6_value_ensemble_a025	rapfi_distillation/round6_regret_restart/training/r6_value_ensemble_a025_eval.pt
r6_value_ensemble_a035	rapfi_distillation/round6_regret_restart/training/r6_value_ensemble_a035_eval.pt
EOF
...
while IFS=$'\t' read -r name checkpoint; do
  ...
  if [[ ! -s "$target/complete.done" ]]; then
    "$PYTHON_BIN" -u -m alphazero_training.rapfi_distill \
      ...
      2>&1 | tee "$RUN/logs/value_ensemble_screen_${name}.log"
...
done < "$RUN/value_ensemble_screen_candidates.tsv"
