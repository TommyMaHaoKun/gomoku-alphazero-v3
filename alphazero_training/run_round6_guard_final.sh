...
CANDIDATE="rapfi_distillation/round6_regret_restart/training/r6_guard_a020_eval.pt"
CANDIDATE_SHA256="222e8efb4f46e7a79450937b039e589f9474ba145d422ff20ad1293b4b517b37"
RUN="rapfi_distillation/round6_regret_restart"
FINAL="$RUN/guard_final_seed20280504"
PAIRS=1024
SEED=20280504
...
run_report() {
  ...
  if [[ ! -s "$target/complete.done" ]]; then
    "$PYTHON_BIN" -u -m alphazero_training.rapfi_distill \
      ...
      2>&1 | tee "$RUN/logs/guard_final_${name}.log"
