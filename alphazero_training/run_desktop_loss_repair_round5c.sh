...
OUTPUT="$RUN/training/last3_repair_s8000.pt"
FROZEN="$RUN/training/last3_repair_s8000_eval.pt"
...
if [[ ! -f "$OUTPUT.done" ]]; then
  "$PYTHON_BIN" -u -m alphazero_training.train_v3_supervised \
    ...
    --dataset rapfi_distillation/longrun_joint_round2/joint_policy_value.npz \
    --dataset rapfi_distillation/longrun_counterfactual_round3/branches.npz \
    --dataset rapfi_distillation/loss_correction_round1/loss_hard_negative.npz \
    ...
    --learning-rate 4e-6 --min-learning-rate 4e-7 --warmup-steps 400 \
    --freeze-trunk-steps 6000 --train-last-residual-blocks-during-freeze 3 \
    --value-loss-scale 0.10 --value-distill-scale 0.25 \
    --policy-distill-scale 1.00 \
    --safe-hard-negative-scale 0.50 --safe-hard-negative-margin 1.0 \
    --mistake-hard-negative-scale 0.80 --mistake-hard-negative-margin 1.0 \
    --random-d4-augmentation --validation-fraction 0.1 --eval-every 2000 \
    --seed 20271130 2>&1 | tee "$RUN/training/train_s8000.log"
...
echo "$?" > "$RUN/gate/desktop_move14.exit"
set -e
...
import sys
r=Path(sys.argv[1]); e=json.loads((r/'gate/desktop_move14.json').read_text())
t=json.loads((r/'gate/tactics.json').read_text()); w=json.loads((r/'gate/white.json').read_text())
s={'desktop_move14_passed':e['passed'],'desktop_teacher_rank':e['raw_network']['teacher_legal_rank'],'desktop_teacher_probability':e['raw_network']['teacher_probability'],'raw_tactics':round(t['raw_network']['top1']*t['samples']),'deployed_tactics':round(t['v3_search_with_exact_oracle']['accuracy']*t['samples']),'white_safe_count':w['metrics']['top1_in_safe_set_count'],'white_safe_probability_mass':w['metrics']['safe_probability_mass']}
s['passed_static_and_exact']=bool(s['desktop_move14_passed'] and s['raw_tactics']>=47 and s['deployed_tactics']>=48 and s['white_safe_count']>=16 and s['white_safe_probability_mass']>=0.7245202151762786)
(r/'gate/summary.json').write_text(json.dumps(s,indent=2)+'\n'); print(json.dumps(s,indent=2))
if not s['passed_static_and_exact']: raise SystemExit('Round5c candidate failed exact or static gate')
PY

echo "[$(date -Is)] Round5c static and exact gates passed; latest.pt unchanged"
