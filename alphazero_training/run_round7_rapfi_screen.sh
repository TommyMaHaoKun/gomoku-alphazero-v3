...
SCREEN="$RUN/rapfi_screen_seed20280806"
PAIRS=256
SEED=20280806
...
cat > "$RUN/rapfi_screen_candidates.tsv" <<'EOF'
round7_a010	rapfi_distillation/round7_league_selfplay/round7_a010_eval.pt
round7_a020	rapfi_distillation/round7_league_selfplay/round7_a020_eval.pt
round7_a035	rapfi_distillation/round7_league_selfplay/round7_a035_eval.pt
EOF
...
run_report() {
  ...
  if [[ ! -s "$target/complete.done" ]]; then
    "$PYTHON_BIN" -u -m alphazero_training.rapfi_distill \
      ...
      2>&1 | tee "$RUN/logs/rapfi_screen_${name}.log"
...
done < "$RUN/rapfi_screen_candidates.tsv"
