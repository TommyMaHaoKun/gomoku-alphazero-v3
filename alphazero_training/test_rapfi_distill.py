...
def test_resume_retries_pairs_containing_engine_errors() -> None:
    black = _complete_record(pair_index=3, student_color=BLACK)
    white = _complete_record(pair_index=3, student_color=WHITE)
    clean_black = _complete_record(pair_index=4, student_color=BLACK)
    clean_white = _complete_record(pair_index=4, student_color=WHITE)
    white.error = "RapfiProtocolError: transient desynchronisation"
    white.termination = "engine_error"
    white.student_result = None
    payload = asdict(white)
    payload.pop("record_sha256")
    white.record_sha256 = canonical_sha256(payload)

    assert successful_pair_indices([black, white, clean_black, clean_white]) == {4}
