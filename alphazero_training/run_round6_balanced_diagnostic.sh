...
SCREEN="$RUN/balanced_diagnostic_seed20280404"
...
cat > "$RUN/balanced_diagnostic_candidates.tsv" <<'EOF'
r6_balanced_a035	rapfi_distillation/round6_regret_restart/training/r6_balanced_a035_eval.pt
r6_balanced_a045	rapfi_distillation/round6_regret_restart/training/r6_balanced_a045_eval.pt
EOF
...
while IFS=$'\t' read -r name checkpoint; do
  ...
  if [[ ! -s "$target/complete.done" ]]; then
    "$PYTHON_BIN" -u -m alphazero_training.rapfi_distill \
      ...
      2>&1 | tee "$RUN/logs/balanced_diagnostic_${name}.log"
...
done < "$RUN/balanced_diagnostic_candidates.tsv"
